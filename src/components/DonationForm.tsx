...
export function DonationForm() {
  ...
  return (
    <form className="journey" onSubmit={submit} ref={formRef}>
      ...
      {step === 0 && (
        <fieldset>
          ...
          <div className="choice-row">
            <button type="button" className={frequency === "one-time" ? "selected" : ""} onClick={() => setFrequency("one-time")}>One-time</button>
            <button type="button" className={frequency === "monthly" ? "selected" : ""} onClick={() => setFrequency("monthly")}>Monthly</button>
          </div>
          ...
          <div className="amounts">
            {suggestedAmounts.map((suggested) => (
              <button type="button" className={amount === String(suggested) ? "selected" : ""} key={suggested} onClick={() => setAmount(String(suggested))}>
          ...
          <div className="step-actions end">
            <button className="button" type="button" onClick={() => continueTo(1)}>Continue</button>
          </div>
        </fieldset>
      )}
      ...
      {step === 1 && (
        <fieldset>
          ...
          <p className="field-help">We need these details to prepare the payment and send an appropriate receipt.</p>
          <div className="two-cols">
            <label>First name<input value={details.firstName} onChange={(event) => updateDetails("firstName", event.target.value)} required /></label>
            <label>Last name<input value={details.lastName} onChange={(event) => updateDetails("lastName", event.target.value)} required /></label>
          </div>
          <label>Email address<input type="email" value={details.email} onChange={(event) => updateDetails("email", event.target.value)} required /></label>
          ...
          <label className="check"><input type="checkbox" checked={details.anonymous} onChange={(event) => updateDetails("anonymous", event.target.checked)} /> Please do not publish my name in supporter recognition.</label>

          {currency === "GBP" && (
            <label className="check"><input type="checkbox" checked={details.giftAid} onChange={(event) => updateDetails("giftAid", event.target.checked)} /> I am a UK taxpayer and want OAKonsult to claim Gift Aid on this donation. I understand that I must have paid enough UK Income Tax or Capital Gains Tax to cover the Gift Aid claimed.</label>
          )}

          <label className="check"><input type="checkbox" checked={details.privacy} onChange={(event) => updateDetails("privacy", event.target.checked)} required /> I have read the privacy information and understand that my details will be used to process this donation.</label>
          <label className="check"><input type="checkbox" checked={details.marketing} onChange={(event) => updateDetails("marketing", event.target.checked)} /> I would also like to receive occasional OAKonsult news and supporter updates. This is optional.</label>
      ...
      {step === 2 && (
        <fieldset>
          ...
          <div className="review">
            ...
            <p className="secure-note">Secure payment will take place with {currency === "GBP" ? "Stripe" : "Paystack"}. OAKonsult will not receive your card details.</p>
          </div>
          <div className="step-actions">
            <button className="button button-muted" type="button" onClick={() => setStep(1)}>Back</button>
            <button className="button" type="submit" disabled={busy}>{busy ? "Preparing…" : "Continue to secure payment"}</button>
          </div>
        </fieldset>
      )}
